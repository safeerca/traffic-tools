...
while true; do
  # Execute remote diagnostic payload over SSH with explicit /dev/tty mapping
  ssh -q -t \
      -o ConnectTimeout=15 \
      -o StrictHostKeyChecking=no \
      "$SSH_USER@$TARGET_IP" \
      "TERM=xterm-256color sudo bash -c '
SERVER_IP=\"$TARGET_IP\"
...
'" < /dev/tty | if [ -n "$LOG_OUTPUT_FILE" ]; then tee -a "$LOG_OUTPUT_FILE"; else cat; fi
